...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for FTM
    ...
    if (p_rewardSnapshot.strDistributionAsset != "FTM") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is FTM with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using FTM differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "FTM") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse FTM address (already validated during ownership snapshot creation)

...
// Copyright (c) 2017-2021 The FTM Core developers
...
const std::string FTMGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString FTMGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "FTMBTC"  , 1,          8},
    { "mBTC",   "FTMBTC"  , 1000,       5},
    { "µBTC",   "FTMBTC"  , 1000000,    2},
    { "Satoshi","FTMBTC"  , 100000000,  0},
    { "USDT",   "FTMUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(FTMGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

FTMGUI::FTMGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void FTMGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a FTM address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ftm: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to FTM addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your FTM addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified FTM addresses"));
    ...
    openAction->setStatusTip(tr("Open a ftm: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible FTM command-line options").arg(tr(PACKAGE_NAME)));
...
void FTMGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/ftmtext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/ftmcointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* ftmLabelLayout = new QVBoxLayout(toolbarWidget);
        ftmLabelLayout->addWidget(labelToolbar);
        ftmLabelLayout->addWidget(m_toolbar);
        ftmLabelLayout->setDirection(QBoxLayout::TopToBottom);
        ftmLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("FTMcoin Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/FTMProject/FTMcoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest FTMcoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/FTMProject/FTMcoin/releases";
...
void FTMGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/ftmtext")));
        m_toolbar->setMaximumWidth(65);
...
void FTMGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool FTMGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FTMGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FTMGUI::setWalletActionsEnabled(bool enabled)
{
...
void FTMGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void FTMGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FTMGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FTMGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void FTMGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void FTMGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void FTMGUI::openClicked()
{
...
void FTMGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FTMGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FTMGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FTMGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FTMGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FTMGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** FTM START */
void FTMGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void FTMGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void FTMGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void FTMGUI::gotoRestrictedAssetsPage()
{
...
void FTMGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to FTM network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void FTMGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FTMGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void FTMGUI::updateHeadersSyncProgressLabel()
{
...
void FTMGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FTMGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("FTM"); // default title
...
void FTMGUI::changeEvent(QEvent *e)
{
...
void FTMGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FTMGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FTMGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "FTM")
        msg += tr("Amount: %1\n").arg(FTMUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(FTMUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void FTMGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FTMGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FTMGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FTMGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FTMGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void FTMGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FTMGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FTMGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FTMGUI::detectShutdown()
{
...
void FTMGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void FTMGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(FTMGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void FTMGUI::subscribeToCoreSignals()
{
...
void FTMGUI::unsubscribeFromCoreSignals()
{
...
void FTMGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void FTMGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<FTMUnits::Unit> units = FTMUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FTMUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(FTMUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(FTMUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (FTMUnits::Unit u : FTMUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FTMUnits::name(u)), this);
...
{
    setText(FTMUnits::name(newUnits));
}
...
void FTMGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FTMGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "FTMGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void FTMGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/FTMProject/FTMcoin/releases"));

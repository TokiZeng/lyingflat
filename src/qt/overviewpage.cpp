...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(FTMUnits::FTM),
    ...
    assetdelegate(new AssetViewDelegate(platformStyle, this))
{
    ...
    ui->ftmBalancesLabel->setStyleSheet(STRING_LABEL_COLOR);
    ...
    ui->ftmBalancesLabel->setFont(GUIUtil::getTopLabelFont());
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(FTMUnits::formatWithUnit(unit, balance, false, FTMUnits::separatorAlways));
    ui->labelUnconfirmed->setText(FTMUnits::formatWithUnit(unit, unconfirmedBalance, false, FTMUnits::separatorAlways));
    ui->labelImmature->setText(FTMUnits::formatWithUnit(unit, immatureBalance, false, FTMUnits::separatorAlways));
    ui->labelTotal->setText(FTMUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance, false, FTMUnits::separatorAlways));
    ui->labelWatchAvailable->setText(FTMUnits::formatWithUnit(unit, watchOnlyBalance, false, FTMUnits::separatorAlways));
    ui->labelWatchPending->setText(FTMUnits::formatWithUnit(unit, watchUnconfBalance, false, FTMUnits::separatorAlways));
    ui->labelWatchImmature->setText(FTMUnits::formatWithUnit(unit, watchImmatureBalance, false, FTMUnits::separatorAlways));
    ui->labelWatchTotal->setText(FTMUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance, false, FTMUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("FTM")
    updateDisplayUnit();
}
...
void OverviewPage::showAssets()
{
    if (AreAssetsDeployed()) {
        ...
        // This keeps the FTM balance grid from expanding and looking terrible when asset balance is hidden

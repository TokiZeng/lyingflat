...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a FTM address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new FTMAddressEntryValidator(parent));
    widget->setCheckValidator(new FTMAddressCheckValidator(parent));
}
...
bool parseFTMURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ftm: URI
    if(!uri.isValid() || uri.scheme() != QString("ftm"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FTMUnits::parse(FTMUnits::FTM, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFTMURI(QString uri, SendCoinsRecipient *out)
{
    // Convert ftm:// to ftm:
    //
    //    Cannot handle this later, because ftm:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ftm://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "ftm:");
    }
    QUrl uriInstance(uri);
    return parseFTMURI(uriInstance, out);
}

QString formatFTMURI(const SendCoinsRecipient &info)
{
    QString ret = QString("ftm:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FTMUnits::format(FTMUnits::FTM, info.amount, false, FTMUnits::separatorNever));
        paramCount++;
    }
...
bool openFTMConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(FTM_CONF_FILENAME);
    ...
    /* Open ftm.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FTM.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "FTM (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("FTM (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ftm.desktop";
    return GetAutostartDir() / strprintf("ftm-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a ftm.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=FTM\n";
        else
            optionFile << strprintf("Name=FTM (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the ftm app
...
bool GetStartOnSystemStartup()
{
    CFURLRef ftmAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (ftmAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, ftmAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ftm app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, ftmAppUrl, nullptr, nullptr);
    }

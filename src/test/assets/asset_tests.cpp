...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of FTMCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("FTM", type));
        BOOST_CHECK(!IsAssetNameValid("FTM", type));
        BOOST_CHECK(!IsAssetNameValid("FTMCOIN", type));

        //- Versions of FTMCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("FTM.COIN", type));
        BOOST_CHECK(IsAssetNameValid("FTM_COIN", type));
        BOOST_CHECK(IsAssetNameValid("FTMSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERFTM", type));
        BOOST_CHECK(IsAssetNameValid("FTMSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEFTM", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_FTMS", type));
        BOOST_CHECK(IsAssetNameValid("SEFTMOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("FTM", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);

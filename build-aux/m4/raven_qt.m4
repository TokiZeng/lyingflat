...
dnl Copyright (c) 2021 The FTMcoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set ftm_enable_qt to false. Else, exit.
AC_DEFUN([FTM_QT_FAIL],[
  if test "x$ftm_qt_want_version" = xauto && test "x$ftm_qt_force" != xyes; then
    if test "x$ftm_enable_qt" != xno; then
      AC_MSG_WARN([$1; ftm-qt frontend will not be built])
    fi
    ftm_enable_qt=no
    ftm_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([FTM_QT_CHECK],[
  if test "x$ftm_enable_qt" != xno && test "x$ftm_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl FTM_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([FTM_QT_PATH_PROGS],[
  FTM_QT_CHECK([
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      FTM_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other FTM_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([FTM_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build ftm-qt GUI (default=auto)])],
    [
     ftm_qt_want_version=$withval
     if test "x$ftm_qt_want_version" = xyes; then
       ftm_qt_force=yes
       ftm_qt_want_version=auto
     fi
    ],
    [ftm_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); ftm_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   FTM_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _FTM_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: ftm_enable_qt, ftm_enable_qt_dbus, ftm_enable_qt_test
AC_DEFUN([FTM_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  FTM_QT_CHECK([_FTM_QT_FIND_LIBS])

  dnl This is ugly and complicated. Yuck. Works as follows:
  dnl For Qt5, we can check a header to find out whether Qt is build
  dnl statically. When Qt is built statically, some plugins must be linked into
  dnl the final binary as well.
  dnl _FTM_QT_CHECK_STATIC_PLUGIN does a quick link-check and appends the
  ...
  _FTM_QT_IS_STATIC
  if test "x$ftm_cv_static_qt" = xyes; then
    _FTM_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _FTM_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _FTM_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      ...
      _FTM_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _FTM_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _FTM_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  FTM_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  FTM_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  FTM_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  FTM_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  FTM_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  ...
  FTM_QT_CHECK([
    ftm_enable_qt=yes
    ftm_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      ftm_enable_qt_test=no
    fi
    ftm_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      ftm_enable_qt_dbus=yes
    fi
  ...
  if test x$ftm_enable_qt = xyes; then
    AC_MSG_RESULT([$ftm_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$ftm_enable_qt])
  fi
...
dnl Output: ftm_cv_static_qt=yes|no
AC_DEFUN([_FTM_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, ftm_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
...
dnl
dnl _FTM_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_FTM_QT_CHECK_STATIC_PLUGIN], [
...
AC_DEFUN([_FTM_QT_FIND_LIBS],[
  FTM_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [],
                      [FTM_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  FTM_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [],
                      [FTM_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  FTM_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [],
                      [FTM_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  FTM_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [],
                      [FTM_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])

...
The RPM spec file provided here is for FTM-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://ftm.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[FTM git repository](https://github.com/FTMProject/FTMcoin) but are not part of
...
Currently only `Source10` is used, for the example `ftm.conf` file.
...
in the ftm git repository. At present only an SVG version of the FTM
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ftm
...
needed when 0.12.1 is released, a proper fix is already in the FTM git
master branch.
...
idea to build FTM Core against a static version of that library compiled
...
The `ftm.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ftm-server` package. At this point, most RPM based
...
The `ftm-server` package should have SELinux support. How to properly do
...
FTM-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ftm/). He has been
packaging FTM for Fedora at least since 2012.
...
4. I renamed the `ftm` package that contains the Qt GUI to `ftm-core` as
that appears to be how the general population refers to it, in contrast to
`ftm-xt` or `ftm-classic`. I wanted to make sure the general population

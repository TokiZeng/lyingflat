...
====================
Some notes on how to build FTM Core in *nix.


Note
---------------------
Always use absolute paths to configure and compile ftm and the dependencies,
...
This will build ftm-qt as well if the dependencies are met.
...
Notes
-----
The release is built with GCC and then "strip ftmd" to strip the debug
...
Security
--------
To help make your ftm installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, ftm should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, ftm may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/ftm/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard FTM Core distributions and independently built

...
The FTM Core daemon can be configured to act as a trusted "border
router", implementing the ftm wire protocol and relay, making
...
The ZeroMQ feature in FTM Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ftmd:
...
For instance:
    ...
    $ ftmd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/ftmd.tx.raw
...
These options can also be provided in ftm.conf.
...
From the perspective of ftmd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ftmd directly. Furthermore, no information is
...
using. FTMd appends an up-counting sequence number to each

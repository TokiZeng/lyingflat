...
`test/functional/test_framework/test_framework.py:FTMTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/ftm_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and ftmd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_ftmd = config["components"].getboolean("ENABLE_FTMD")
    if not (enable_wallet and enable_cli and enable_ftmd):
        print("No functional tests to run. Wallet, utils, and ftmd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if ftmd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "ftmd"]) is not None:
            print("%sWARNING!%s There is already a ftmd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "FTMD" not in os.environ:
        os.environ["FTMD"] = build_dir + '/src/ftmd' + exeext
        os.environ["FTMCLI"] = build_dir + '/src/ftm-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `ftm-cli help` (`rpc_interface.txt`).
